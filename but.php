...
echo<<<END
                    <div class="komentarz">
                        ...
                        <div class="opss{$k['id']}" style="background-color: #f5f5f5;margin-bottom: 10px;">
END;
                    $comment = $k['opis'];
                    if(strlen($comment)>500){
                        $stringCut = substr($comment, 0, 500);
                        $stringmore = substr($comment,500);
                        $forem = "... <form method='POST' style='background-color:#f5f5f5;color:black;padding:0; width:auto;'><input type='submit' id='more' name='dalej' style='background-color:#f5f5f5;color:black;padding:0;' value='Pokaż więcej'></form>";
                        echo $forem;

                        if($_POST['dalej'] == "Pokaż więcej"){
                           
echo<<<END
                            <script>
                            $('#more').val("Pokaż mniej");
                            $('.opss{$k['id']}').html("UDALO SIE {$forem}");
                            </script>
END;
                        }
